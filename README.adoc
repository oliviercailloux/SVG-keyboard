...
One goal is to help improve the key bindings display that https://unix.stackexchange.com/a/368089/[used to be], or https://gitlab.gnome.org/GNOME/tecla[is], included in Gnome.

The file encoding of the behavior of the keyboard is limited to what the operating system sees. It is not a goal to represent the internal electronics of the keyboard, such as how “https://en.wikipedia.org/wiki/Fn_key#Technical_details[meta-modifier]” keys change the signals that other keys send to the operating system.
...
The intent is to allow powerful features of the SVG language to be used to represent a keyboard by any means that the designer deems useful. In such a way, complex keyboards can be represented, with different key sizes, colors, and shapes, with no need of new complex file formats.

This can be used to represent the association between keyboard physical keys and Wayland (as defined in https://github.com/xkbcommon/libxkbcommon/blob/master/doc/introduction-to-xkb.md[keymap] files); or Emacs shortcuts; or any other key bindings.
...
The signals would perhaps be better described using a standard that is not tightly related to X, to make it clear that it is not specific to Unix related systems. We could consider using the https://docs.qmk.fm/keycodes_basic[QMK key names] instead of X key names. But I think they are much less https://docs.rs/qmk-via-api/latest/qmk_via_api/keycodes/enum.Keycode.html[numerous]. Perhaps something coming from the USB specification. For this I need help from USB experts.
...
This proposal defines no standard for writing the key caps in the file. This can be done by using a https://github.com/oliviercailloux/XKB-doc/blob/main/README.adoc[keymap] that associates X key names to key caps.
